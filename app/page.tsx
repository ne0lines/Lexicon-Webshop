...
export default async function Home({
  ...
  const { products: allProducts, total: totalProducts }: ProductsResponse =
    await fetch(
      `${API_URL}/products/?_sort=id&_order=desc&_expand=category`,
    ).then((res) => res.json());

  const { products, total, page, pages, limit }: ProductsResponse = await fetch(
    `${API_URL}/products/?_limit=${defaultLimit}&_sort=id&_order=desc&_expand=category&_page=${currentPage}&title_like=${search}`,
  ).then((res) => res.json());
