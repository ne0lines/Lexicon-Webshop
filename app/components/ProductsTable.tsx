...
export default function ProductsTable({
    products,
    categories,
}: Readonly<{ products: ProductsResponse["products"]; categories: Category[] }>) {
    ...
    useEffect(() => {
        ...
        globalThis.addEventListener(OPEN_CREATE_PRODUCT_EVENT, onOpenCreateFromHeader);

        return () => {
            globalThis.removeEventListener(OPEN_CREATE_PRODUCT_EVENT, onOpenCreateFromHeader);
        };
    }, []);
    ...
    async function handleSubmitEdit(values: ProductFormValues) {
        ...
        try {
            ...
            setErrorMessage(
                error instanceof Error ? error.message : "Något gick fel vid uppdatering.",
            );
        } finally {
            setIsSubmitting(false);
        }
